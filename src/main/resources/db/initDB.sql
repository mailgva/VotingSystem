...
CREATE OR REPLACE FUNCTION GENERATE_DAILYMENU(fromdate date, todate date)
...
BEGIN
  ...
  FOR D IN 0..DAYS LOOP
    ...
    FOR R IN 1..(ARRAY_LENGTH(RESTS, 1)) LOOP
      CURREST := RESTS[R];
      INSERT INTO DAILY_MENU(ID, DATE, REST_ID) VALUES(NEXTVAL('GLOBAL_SEQ'), CURDATE, CURREST) RETURNING ID INTO NEW_ID;

      FOR I IN 0..4 LOOP
        DISHES := ARRAY(
            SELECT DISHES.ID FROM DISHES
            WHERE NOT DISHES.ID IN
                      (SELECT DAILY_MENU_DETAIL.DISH_ID
                       FROM DAILY_MENU
                       JOIN DAILY_MENU_DETAIL ON DAILY_MENU.ID = DAILY_MENU_DETAIL.DAILY_MENU_ID
                       WHERE DAILY_MENU.DATE = CURDATE AND DAILY_MENU.REST_ID = CURREST)
          );

        DISH := DISHES[FLOOR(RANDOM() * ARRAY_LENGTH(DISHES, 1)+1)];

        INSERT INTO DAILY_MENU_DETAIL(ID, DAILY_MENU_ID, DISH_ID) VALUES(NEXTVAL('GLOBAL_SEQ'), NEW_ID, DISH);
      END LOOP;
...
CREATE OR REPLACE FUNCTION generate_strdailymenu(
    fromdatestr character varying,
    todatestr character varying)
  RETURNS integer AS
$BODY$
DECLARE
  fromdate date;
...
BEGIN
  ...
  PERFORM generate_dailymenu(fromdate, todate);
...
ALTER FUNCTION public.generate_dailymenu(character varying, character varying)
